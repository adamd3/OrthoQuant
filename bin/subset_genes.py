...
def find_core(
    gene_presence_absence, metadata_merged, perc, rm_split, ref_only, outf
):
    ...
    clone_sub = clone_data[list(set(clone_names))].copy(deep=True)
    ...
    max_na = len(clone_sub.columns) - (
        round((len(clone_sub.columns) / 100) * float(perc))
    )
